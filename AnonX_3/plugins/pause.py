...
async def _pause(_, m: types.Message):
    ...
    tpl = await db.get_custom_text("play_paused", m.lang["play_paused"], lang_code)
    tpl = await utils.normalize_template_entities(
        "play_paused", tpl, lang_code=lang_code
    )
    ...
    if media and media.message_id:
        ...
        keyboard = buttons.controls(m.chat.id, status=m.lang["paused"], status_style="danger")
        try:
        ...
        await utils.reply_text(
            ...
            text=text,
            entities=entities,
            reply_markup=buttons.controls(m.chat.id, status=m.lang["paused"], status_style="danger"),
        )
